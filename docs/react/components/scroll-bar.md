...
`ScrollBar` accepts standard `HTMLAttributes<HTMLDivElement>` for the outer wrapper, plus:
...
| `scrollerProps` | `HTMLAttributes<HTMLDivElement>` | — | Props applied to the inner scrollable element (the first child). |

When `vertical` or `horizontal` is an object, it can include:

- **trackProps** — passed to the track element (e.g. `className`, `style`).
- **thumbProps** — passed to the thumb element.
...
See [VerticalScrollBar](/react/components/vertical-scroll-bar) and [HorizontalScrollBar](/react/components/horizontal-scroll-bar) for the building blocks used inside `ScrollBar`.
